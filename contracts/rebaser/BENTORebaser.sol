...
import "../token/BENTOTokenInterface.sol";


contract BENTORebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 bentosSold, uint256 bentosFromReserves, uint256 bentosToReserves);
    ...
    /// @notice BENTO token address
    address public bentoAddress;
    ...
    /// @notice pair for reserveToken <> BENTO
    ...
    /// @notice Whether or not this token is first in uniswap BENTO<>Reserve pair
    bool public isToken0;

    constructor(
        address bentoAddress_,
        ...
        public
    {
          ...
          if (token0 == bentoAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap BENTO<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        BENTOTokenInterface bento = BENTOTokenInterface(bentoAddress);

        if (positive) {
            require(bento.bentosScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < bento.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = bento.totalSupply();
        ...
        // rebase
        uint256 supplyAfterRebase = bento.rebase(epoch, indexDelta, positive);
        assert(bento.bentosScalingFactor() <= bento.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            bento.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.bentosToUni) {
                // if the amount from reserves > bentosToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                bento.mint(uniswap_pair, uniVars.bentosToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            bento.mint(uniswap_pair, uniVars.bentosToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            bento.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.bentosToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.bentosToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess bento in the reserve
        uint256 excess = bento.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          bentosToUni: tokens_to_max_slippage, // how many bentos uniswap needs
          amountFromReserves: excess, // how much of bentosToUni comes from reserves
          mintToReserves: 0 // how much bentos protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.bentosToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire bento reserves, and minting part of total
