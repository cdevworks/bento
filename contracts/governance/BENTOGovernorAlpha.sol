...
// Modified to work in the BENTO system

// all votes work on underlying _bentoBalances[address], not balanceOf(address)
...
//
// Changes made by BENTO after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since BENTO supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match BENTO as opposed to comp
...
contract GovernorAlpha {
    ...
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(bento.initSupply(), 4), 100); } // 4% of BENTO

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(bento.initSupply(), 100); } // 1% of BENTO
    ...
    constructor(address timelock_, address bento_) public {
        timelock = TimelockInterface(timelock_);
        bento = BENTOInterface(bento_);
        guardian = msg.sender;
    }
