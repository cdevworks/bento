...
import { getProposal, getQuorumVotes, getProposalStatus, castVote } from '../../bentoUtils'
...
const ProposalPage: React.FC = () => {
  ...
  const handleVoteForClick = useCallback(() => {
    castVote(bento, proposal.id, true, account )
  }, [account, bento, proposal.id])

  const handleVoteAgainstClick = useCallback(() => {
    castVote(bento, proposal.id, false, account )
  }, [account, bento, proposal.id])

  const fetchProposal = useCallback(async () => {
    const proposal = await getProposal(bento, proposalId)
    setProposal(proposal)
  }, [bento, proposalId])

  const fetchVotes = useCallback(async () => {
    const proposalStatus:ProposalStatus = await getProposalStatus(bento, proposalId)
    ...
    const quorumCount = await getQuorumVotes(bento)
    setVotes({
  ...
  }, [fetchProposal, fetchVotes, bento])
